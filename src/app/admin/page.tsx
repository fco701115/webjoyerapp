...
export default function AdminDashboard() {
    ...
    const getStatusStyle = (status: string) => {
        switch (status) {
            ...
            case 'PROCESSING': return 'text-green-600 bg-green-100';
                    ...
                    <div className="flex items-center gap-3">
                        <Link 
                            ...
                            className={`h-14 px-6 bg-white rounded-2xl border shadow-sm font-bold text-[13px] text-slate-600 hover:bg-slate-50 transition-all flex items-center gap-2 relative ${
                                ...
                                ? 'border-green-200 ring-4 ring-green-500/10 animate-pulse' 
                                : 'border-slate-100'
                            }`}
                        >
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
                {statsConfig.map((stat, i) => (
                    <Link key={i} href={stat.href} className="bg-white p-4 sm:p-8 rounded-[24px] sm:rounded-[40px] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 group relative overflow-hidden">
                        <div className="absolute -top-10 -right-10 w-24 h-24 sm:w-40 sm:h-40 bg-slate-50 rounded-full group-hover:bg-green-50/50 transition-colors duration-500" />
                        ...
                        {data?.revenueByMonth?.map((item: any, i: number) => {
                            ...
                            return (
                                <div key={i} className="flex-grow group relative h-full flex flex-col justify-end items-center">
                                    ...
                                    <div
                                        className="w-full max-w-[60px] bg-gradient-to-t from-green-600 to-pink-400 rounded-2xl transition-all duration-1000 ease-out shadow-lg shadow-green-500/10 group-hover:shadow-green-500/20 group-hover:to-pink-300 relative overflow-hidden"
